...
impl ProposalsContract {
    ...
    pub(crate) fn assert_only_operator(&self) {
        require!(
            self.admin_id == env::signer_account_id()
                || self.operator_ids.contains(&env::signer_account_id()),
            "Only the operator or admin can call this function."
        );
    }

    pub(crate) fn assert_only_operator_or_creator(&self, proposal_id: ProposalId) {
    ...
    pub(crate) fn assert_only_creator(&self, proposal_id: ProposalId) {
        let proposal = self.internal_get_proposal(&proposal_id);
        require!(
            proposal.creator_id == env::signer_account_id(),
    ...
    pub(crate) fn assert_open_for_new_proposals(&self) {
        require!(
            self.open_for_new_proposals == true,
            "Contract not open for new proposals"
        );
    }

    pub(crate) fn internal_has_voted(&self, proposal_id: &ProposalId, voter_id: &VoterId) -> bool {
        let voter = self.voters.get(&voter_id);
        match voter {
            Some(_voter) => !_voter.votes.get(&proposal_id).is_none(),
            None => false,
        }
    }

    pub(crate) fn assert_has_not_voted(&self, proposal_id: ProposalId, account_id: VoterId) {
        require!(
            !self.internal_has_voted(&proposal_id, &account_id),
            "Account has already voted"
        );
    }

    pub(crate) fn assert_has_voted(&self, proposal_id: ProposalId, account_id: VoterId) {
        require!(
            self.internal_has_voted(&proposal_id, &account_id),
    ...
    pub(crate) fn internal_proposal_is_active(&self, proposal_id: ProposalId) -> bool {
        let proposal = self.internal_get_proposal(&proposal_id);
    ...
    pub(crate) fn internal_proposal_is_active_or_draft(&self, proposal_id: ProposalId) -> bool {
        let proposal = self.internal_get_proposal(&proposal_id);
        // check if proposal has vote_start_timestamp
        match proposal.vote_start_timestamp {
            Some(date) => {
    ...
    pub(crate) fn internal_proposal_is_on_voting(&self, proposal_id: &ProposalId) -> bool {
        let proposal = self.internal_get_proposal(&proposal_id);
        log!("proposal {} on voting?", proposal.proposal_id.to_string());
        // check if proposal has vote_start_timestamp
        match proposal.vote_start_timestamp {
            Some(date) => {
                get_current_epoch_millis() >= date
    ...
    pub(crate) fn assert_proposal_is_active_or_draft(&self, proposal_id: ProposalId) {
        require!(
    ...
    pub(crate) fn assert_proposal_is_on_voting(&self, proposal_id: &ProposalId) {
        require!(
            self.internal_proposal_is_on_voting(&proposal_id),
            "Proposal is not on voting period"
        )
    }

    pub(crate) fn internal_get_proposal(&self, proposal_id: &ProposalId) -> Proposal {
        self.proposals
            .get(&proposal_id)
            .expect("Proposal Id does not exist")
    }

    pub(crate) fn internal_get_proposal_vote(&self, proposal_id: ProposalId) -> ProposalVote {
        self.votes
            .get(&proposal_id)
            .unwrap_or(ProposalVote::new(&proposal_id))
    }

    pub(crate) fn internal_get_voter_vote(&self, proposal_id: &ProposalId, voter_id: &VoterId) -> Vote {
        let voter = self
            .voters
            .get(&voter_id)
    ...
    pub(crate) fn internal_get_total_voting_power_from_promise(&self) -> Balance {
        ...
        match env::promise_result(0) {
            ...
            PromiseResult::Failed => env::panic_str("Staking Position is not available!"),
    ...
    pub(crate) fn internal_is_quorum_reached(&self, proposal_id: ProposalId) -> bool {
    ...
    pub(crate) fn internal_get_proposer(&self, proposer_id: AccountId) -> Vec<ProposalId> {
        self.proposers
            .get(&proposer_id)
